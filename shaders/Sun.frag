...
void main() {
   ...
   /*
   Diffuse = directionalColor * vec3(0.6, 0.6, 0.7) * dotNLDir;
   Spec = directionalColor * vec3(0.2, 0.2, 0.3) * pow(dotVRDir, uMat.shine);
   color =  Diffuse + vec3(vec4(Spec, 1.0) * uViewMatrix) + vec3(0.2, 0.2, 0.2);
   */
